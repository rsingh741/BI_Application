...
PATH = os.path.join(os.path.dirname( __file__ ), 'data_files/retail/')
...
def create_and_dump_item_from_csv():
    """ fot default retail item tables"""
    client_id = 1
    category = 'item'
    df = pd.read_csv(PATH + 'mst_item.csv')    
    df.to_pickle(PATH + f'{client_id}_{category}.pkl')
    
def create_and_dump_sales_from_csv():
    
    client_id = 1
    category = 'sales'
    df = pd.read_csv(PATH + 'sales_details.csv')    
    df.to_pickle(PATH + f'{client_id}_{category}.pkl')
